...
class SearchEngineSpec extends FunSpec with Matchers {
  describe("SearchEngine") {
    ...
    /**
      * Ecris les tests au fur et à mesure des évolutions du moteur de recherche
      */
    ...
    describe("AdapterSearch") {
      ...
      it("should search for custom documents") {
        search(questionAdapter, db.take(100))("sql").length shouldBe 1
      }
    }
    describe("TFIDF") {
    ...
    describe("GlobalScoring") {
      ...
      it("should calculate df once and the search fast") {
        ...
        engine.search("sql").length shouldBe 1
      }
    }
    describe("LazyLoad") {
